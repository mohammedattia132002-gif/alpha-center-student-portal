...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  // Active notifications will only be triggered by real application events
  useEffect(() => {
    // Simulated alerts removed for production launch
  }, []);
  ...
  return (
    <ToastContext.Provider value={{ toasts, showToast, dismissToast }}>
      ...
      <div className="fixed bottom-6 left-6 z-50 flex flex-col gap-3 w-full max-w-sm pointer-events-none px-4 md:px-0" dir="rtl">
        <AnimatePresence>
          {toasts.map((toast) => (
            <motion.div
              key={toast.id}
